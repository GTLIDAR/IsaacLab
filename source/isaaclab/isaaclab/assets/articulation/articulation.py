...
class Articulation(AssetBase):
    ...
    def write_joint_position_limit_to_sim(
        ...
        self.root_physx_view.set_dof_limits(
            self._data.joint_pos_limits.cpu(), indices=physx_env_ids.cpu()
        )
    ...
    def _initialize_impl(self):
        ...
        if self.cfg.articulation_root_prim_path is not None:
            ...
            root_prim_path_expr = (
                self.cfg.prim_path + self.cfg.articulation_root_prim_path
            )
        else:
            ...
            root_prim_path_expr = (
                self.cfg.prim_path + root_prim_path_relative_to_prim_path
            )
    ...
    def _create_buffers(self):
        ...
        # -- joint properties
        self._data.default_joint_pos_limits = (
            self.root_physx_view.get_dof_limits().to(self.device).clone()
        )
        ...
        self._data.joint_vel_limits = (
            self.root_physx_view.get_dof_max_velocities().to(self.device).clone()
        )
        self._data.joint_effort_limits = (
            self.root_physx_view.get_dof_max_forces().to(self.device).clone()
        )
        ...
        self._data.joint_friction_coeff = (
            self._data.default_joint_friction_coeff.clone()
        )
    ...
    def _process_actuators_cfg(self):
        ...
        for actuator_name, actuator_cfg in self.cfg.actuators.items():
            ...
            self._data.default_joint_armature[:, actuator.joint_indices] = (
                actuator.armature
            )
            self._data.default_joint_friction_coeff[:, actuator.joint_indices] = (
                actuator.friction
            )
    ...
    def _process_fixed_tendons(self):
        ...
        if self.num_fixed_tendons > 0:
            ...
            self._data.default_fixed_tendon_pos_limits = (
            ...
            self._data.fixed_tendon_damping = (
                self._data.default_fixed_tendon_damping.clone()
            )
            self._data.fixed_tendon_limit_stiffness = (
                self._data.default_fixed_tendon_limit_stiffness.clone()
            )
            self._data.fixed_tendon_pos_limits = (
                self._data.default_fixed_tendon_pos_limits.clone()
            )
            self._data.fixed_tendon_rest_length = (
                self._data.default_fixed_tendon_rest_length.clone()
            )
            self._data.fixed_tendon_offset = (
                self._data.default_fixed_tendon_offset.clone()
            )
    ...
    def _log_articulation_info(self):
        ...
        joint_table.title = (
            f"Simulation Joint Information (Prim path: {self.cfg.prim_path})"
        )
        ...
        joint_table.custom_format["Position Limits"] = (
            lambda f, v: f"[{v[0]:.3f}, {v[1]:.3f}]"
        )
        ...
        if self.num_fixed_tendons > 0:
            ...
            tendon_table.title = (
                f"Simulation Fixed Tendon Information (Prim path: {self.cfg.prim_path})"
            )
            ...
            joint_table.custom_format["Limits"] = (
                lambda f, v: f"[{v[0]:.3f}, {v[1]:.3f}]"
            )
            # add info on each term
    ...
    def set_fixed_tendon_limit(
        ...
        self.set_fixed_tendon_position_limit(
            limit, fixed_tendon_ids=fixed_tendon_ids, env_ids=env_ids
        )
