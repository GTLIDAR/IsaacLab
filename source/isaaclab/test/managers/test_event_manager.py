...
DummyEnv = namedtuple(
    "ManagerBasedRLEnv", ["num_envs", "dt", "device", "sim", "dummy1", "dummy2"]
)
...
def test_str(env):
    ...
    cfg = {
        ...
        "term_3": EventTermCfg(
            func=change_dummy1_by_value, mode="custom", params={"value": 10}
        ),
        "term_4": EventTermCfg(
            func=change_dummy1_by_value, mode="custom", params={"value": 2}
        ),
    }
...
def test_config_equivalence(env):
    ...
    class MyEventManagerAnnotatedCfg:
        ...
        term_3: EventTermCfg = EventTermCfg(
            func=change_dummy1_by_value, mode="custom", params={"value": 10}
        )
    ...
    # parsed term configs
    assert (
        event_man_from_dict._mode_term_cfgs
        == event_man_from_annotated_cfg._mode_term_cfgs
    )
    assert (
        event_man_from_cfg._mode_term_cfgs
        == event_man_from_annotated_cfg._mode_term_cfgs
    )
...
def test_class_terms(env):
    ...
    cfg = {
        ...
        "term_2": EventTermCfg(
            func=increment_dummy2_by_one_class,
            mode="interval",
...
def test_apply_interval_mode_without_global_time(env):
    ...
    for count in range(50):
        ...
        torch.testing.assert_close(
            env.dummy1, (count + 1) // 10 * torch.ones_like(env.dummy1)
        )
        ...
        if (count + 1) % 10 == 0:
            ...
            torch.testing.assert_close(
                term_1_interval_time_init, expected_time_interval_init
            )
        # -- random interval
        env_ids = (term_2_interval_time < 1e-6).nonzero(as_tuple=True)[0]
        if len(env_ids) > 0:
            term_2_interval_time[env_ids] = event_man._interval_term_time_left[1][
                env_ids
            ]
...
def test_apply_reset_mode(env):
    ...
    cfg = {
        ...
        "term_2": EventTermCfg(
            func=reset_dummy1_to_zero, mode="reset", min_step_count_between_reset=10
        ),
    }
    ...
    term_2_trigger_step_id = torch.zeros(
    ...
    for count in range(50):
        ...
        torch.testing.assert_close(
            event_man._reset_term_last_triggered_step_id[0], expected_trigger_count
        )
        # -- term 2
        torch.testing.assert_close(
            event_man._reset_term_last_triggered_step_id[1], term_2_trigger_step_id
        )
...
def test_apply_reset_mode_subset_env_ids(env):
    ...
    term_2_trigger_once = torch.zeros(
    ...
    for count in range(50):
        ...
        env_ids = (
            (torch.rand(env.num_envs, device=env.device) < 0.5).nonzero().flatten()
        )
        ...
        trigger_ids |= (term_2_trigger_step_id[env_ids] == 0) & ~term_2_trigger_once[
            env_ids
        ]
        ...
        # -- term 1
        expected_trigger_count = torch.full(
            (len(env_ids),), count, dtype=torch.int32, device=env.device
        )
        torch.testing.assert_close(
