...
@hydra.main(config_path="", config_name="config_isaaclab", version_base="1.1")
def main(cfg: "DictConfig"):  # noqa: F821 # type: ignore
    ...
    if logger_video:
        test_env = test_env.append_transform(
            VideoRecorder(logger, tag="rendering/test", in_keys=["pixels"])  # type: ignore
        )
    test_env.eval()
    ...
    for i, data in enumerate(collector):
        ...
        # Get test rewards
        with torch.no_grad(), set_exploration_type(ExplorationType.DETERMINISTIC):  # type: ignore
